  ...
  return (
    <div className="tbl-overlay" onClick={onClose}>
      <div className="tbl-dialog" onClick={e => e.stopPropagation()} style={{ minWidth: 520, maxWidth: 620, maxHeight: '80vh', overflowY: 'auto' }}>
        ...
        {loading && (
          <div style={{ textAlign: 'center', padding: '30px', color: '#059669', fontSize: 15 }}>
            جاري التحميل...
          </div>
        )}
        {error && (
          <div style={{ textAlign: 'center', padding: '20px', color: '#dc2626', fontSize: 13 }}>
            ⚠️ {error}
          </div>
        )}
          ...
          >
            <div style={{ flex: 1, overflow: 'hidden' }}>
              ...
              <div style={{ fontSize: 11, color: '#6b7280' }}>
                آخر تعديل: {formatDate(doc.updatedAt)}
              </div>
            </div>
            <button
...
export default function DocumentEditor() {
  ...
  const getCleanHTMLForSave = () => {
    ...
    const selectorsToRemove = [
      '.resize-handle', '.resize-row-handle', '.row-move-handle',
      '.tbl-corner-handle', '.table-move-handle', '.table-resize-btn',
    ];
  ...
  const saveDocument = useCallback(async (silent = false) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success && data.document?._id) {
        setCurrentDocId(data.document._id);
      }
      setSaveStatus('saved');
  ...
  const newDocument = () => {
    if (editorRef.current?.innerText.trim()) {
      if (!window.confirm('هل تريد بدء مستند جديد؟ سيتم فقدان التغييرات غير المحفوظة.')) return;
    }
    setTitle('');
  ...
  const addResizeHandles = (table) => {
    ...
    table.querySelectorAll('tr').forEach((row) => {
      ...
      rh.style.cssText = [
        ...
        'height:6px', 'cursor:row-resize', 'z-index:30', 'background:transparent',
  ...
  // ─── دالة التنظيف للطباعة (المحسنة جداً) ─────────────────────────────────────
  const getCleanHTML = () => {
    const ed = editorRef.current;
    if (!ed) return '';
    const clone = ed.cloneNode(true);
    ...
    // معالجة الجداول بشكل محسن للطباعة
    clone.querySelectorAll('.movable-table-wrapper').forEach(wrapper => {
      const table = wrapper.querySelector('table');
      if (table) {
        const tableClone = table.cloneNode(true);
        selectorsToRemove.forEach(sel => tableClone.querySelectorAll(sel).forEach(el => el.remove()));
        ...
        tableClone.querySelectorAll('th, td').forEach(cell => {
          ...
          const bgColor = isTh ? '#f0f0f0' : (cell.style.backgroundColor || cell.style.background || '#ffffff');
        ...
        cleanDiv.style.cssText = 'display: block; width: 100%; margin: 25px 0; clear: both; page-break-inside: avoid;';
        cleanDiv.appendChild(tableClone);
        wrapper.replaceWith(cleanDiv);
      } else {
        wrapper.remove();
      }
    });
...
<style>
  ...
  * { box-sizing: border-box; }
  body { font-family: 'Cairo', sans-serif; line-height: 1.9; color: #111; background: #fff; margin: 0; padding: 0; }
  .print-page { max-width: 830px; margin: 0 auto; padding: 25mm 22mm; background: white; }
  .print-hdr { border-bottom: 6px solid #10b981; padding-bottom: 20px; margin-bottom: 35px; }
  .print-hdr-brand { color: #10b981; font-size: 30px; font-weight: 900; }
  .print-hdr-sub { color: #0f766e; font-weight: 600; font-size: 14.5px; }
  .doc-title { text-align: center; color: #10b981; font-size: 26px; font-weight: 900; margin: 30px 0 20px; }
  .resize-handle,.resize-row-handle,.row-move-handle,.tbl-corner-handle,.table-move-handle,.table-resize-btn { display: none !important; }
  table { border-collapse: collapse !important; margin: 25px 0 !important; table-layout: fixed !important; page-break-inside: avoid !important; direction: rtl !important; width: 100% !important; }
  th, td { border: 1.6px solid #000 !important; text-align: center !important; vertical-align: middle !important; word-break: break-word !important; direction: rtl !important; position: static !important; outline: none !important; font-family: 'Cairo', sans-serif !important; }
  th { font-weight: 700 !important; color: #000 !important; }
  tr { page-break-inside: avoid !important; }
  thead { display: table-header-group !important; }
  .content-body p { margin: 12px 0; white-space: normal; word-break: break-word; }
...
<div class="no-print" style="position:fixed;bottom:30px;left:50%;transform:translateX(-50%);background:#0f172a;color:white;padding:16px 50px;border-radius:12px;box-shadow:0 8px 30px rgba(0,0,0,0.5);">
  ...
  const renderSaveStatus = () => {
    ...
    const cfg = {
      saving: { bg: '#fef9c3', color: '#854d0e', icon: '⏳', text: 'جاري الحفظ...' },
      saved: { bg: '#d1fae5', color: '#065f46', icon: '✓', text: 'تم الحفظ' },
      error: { bg: '#fee2e2', color: '#991b1b', icon: '✕', text: 'خطأ في الحفظ' },
    };
    const c = cfg[saveStatus];
    if (!c) return null;
    return (
      <span style={{
...
.btn-new{background:#7c3aed;color:#fff}
...
.resize-row-handle{position:absolute;bottom:0;left:0;height:6px;cursor:row-resize;z-index:30;background:transparent;transition:background .15s;width:9999px;}
      ...
      <div className="de-page">
        <div className="de-header">
          ...
          <div className="de-actions">
            ...
            />
            <button className="de-btn btn-new" onClick={newDocument} title="مستند جديد">
              ✦ جديد
            </button>
            <button className="de-btn btn-save" onClick={() => saveDocument(false)} title="حفظ (Ctrl+S)">
              💾 حفظ
            </button>
            <button className="de-btn btn-load" onClick={() => setShowSavedDocs(true)} title="فتح مستند محفوظ">
              📂 فتح
            </button>
            {renderSaveStatus()}
            <div className="tb-sep" style={{ height: 28, margin: '0 4px' }} />
        ...
        <div className="de-toolbar" style={{ maxWidth: `${pageWidth}px` }}>
          ...
          <span style={{ fontSize: 11, color: '#94a3b8', padding: '0 4px', whiteSpace: 'nowrap', direction: 'rtl' }}>
            Tab ← خلية التالية &nbsp;|&nbsp; ↑↓←→ ← تنقل بين الصفوف &nbsp;|&nbsp; Ctrl+S ← حفظ
          </span>
        </div>
        ...
        <div className="de-status">
          <span>
            كلمات: {wordCount} · أحرف: {charCount}
            {currentDocId && <span style={{ color: '#34d399', marginRight: 8 }}>· محفوظ في قاعدة البيانات</span>}
          </span>
